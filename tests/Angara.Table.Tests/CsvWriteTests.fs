        ...
        t |> Seq.forall(fun c -> 
            ((match c.Rows with StringColumn _ -> true | _ -> false) &&
             let strings = c.Rows.AsString in 
                strings |> Seq.forall(fun s -> s = null || let r = ref 0.0 in System.Double.TryParse(s, r)) ||
...
[<Test; Category("CI")>]
let ``When reading the written table we get a table that equals original table - case 9`` () =
    Table.Empty 
    |> Table.Add (Column.Create("h", [| "5"; null |])) 
    |> CheckReadWrite { DelimitedFile.ReadSettings.Default with InferNullStrings = true; DelimitedFile.ColumnTypes = Some(fun _ -> Some(typeof<string>)) } { DelimitedFile.WriteSettings.Default with AllowNullStrings = true }

[<Test; Category("CI")>]
let ``When reading the written table we get a table that equals original table - case 8`` () =
    Table.Empty 
    |> Table.Add (Column.Create("0&", [| true; true |])) 
    |> Table.Add (Column.Create("", [| null; "" |])) 
...
let ``When reading the written table we get a table that equals original table - case 7`` () =
    Table.Empty |> Table.Add (Column.Create("a", [| "10" |])) |> CheckReadWrite { DelimitedFile.ReadSettings.Default with DelimitedFile.ColumnTypes = Some(fun _ -> Some(typeof<string>)) } DelimitedFile.WriteSettings.Default

[<Test; Category("CI")>]
let ``When reading the written table we get a table that equals original table - case 6`` () =
    Table.Empty 
    |> Table.Add (Column.Create(new System.String([| '\009'; '\029'; 'o'; '\001'; 'w'; '\025' |]), [| false; true; false; false |]))
    |> Table.Add (Column.Create("", [| true; false; true; true |]))
...
let ``When reading the written table we get a table that equals original table - case 5`` () =
    Table.Empty 
    |> Table.Add (Column.Create(new System.String([| '\020' |]), [| System.DateTime(69, 8, 6, 14, 04, 56, 132) |]))
    |> Table.Add(Column.Create("", [|true|]))
    |> CheckReadWrite DelimitedFile.ReadSettings.Default DelimitedFile.WriteSettings.Default

[<Test; Category("CI")>]
let ``When reading the written table we get a table that equals original table - case 4`` () =
    Table.Empty |> Table.Add (Column.Create(new System.String([| '\009' |]), [| 1.0 + 2.0/3.0; 1.0 |])) |> CheckReadWrite DelimitedFile.ReadSettings.Default DelimitedFile.WriteSettings.Default

[<Test; Category("CI")>]
let ``When reading the written table we get a table that equals original table - case 3`` () =
    Table.Empty 
    |> Table.Add (Column.Create("Prh", [| 2.0; -2.0; 0.0; 2.0; -1.0 |]))
    |> Table.Add(Column.Create("", [| true; true; false; true; false |] ))
    |> CheckReadWrite DelimitedFile.ReadSettings.Default DelimitedFile.WriteSettings.Default

[<Test; Category("CI")>]
let ``When reading the written table we get a table that equals original table - case 1`` () =
    Table.Empty |> Table.Add (Column.Create("Q", Array.empty<string>)) |> CheckReadWrite DelimitedFile.ReadSettings.Default DelimitedFile.WriteSettings.Default

[<Test; Category("CI")>]
let ``When reading the written table we get a table that equals original table - case 2`` () =
    Table.Empty |> Table.Add (Column.Create("Q", Array.empty<float>)) |> CheckReadWrite { DelimitedFile.ReadSettings.Default with DelimitedFile.ColumnTypes = Some(fun _ -> Some(typeof<float>)) } DelimitedFile.WriteSettings.Default

[<Test; Category("CI")>]
let ``When reading the written table we get a table that equals original table - empty column name`` () =
    Table.Empty |> Table.Add (Column.Create("", [|-2.0|])) |> CheckReadWrite DelimitedFile.ReadSettings.Default DelimitedFile.WriteSettings.Default
