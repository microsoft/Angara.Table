...
let ``Table.ToRows for a record``() =
    let t = Table.OfColumns ([  Column.Create("TypeString", [|"Abcdef"|])
                                Column.Create("TypeInt", [|45|])
                                Column.Create("TypeFloat", [|185.0|])
                                Column.Create("TypeBool", [|true|])
                                Column.Create("TypeDate", [|System.DateTime(1950, 02, 03)|])])
...
let ``Table.ToRows for a non-record``() =
    let t = Table.OfColumns ([  Column.Create("Prop", [|"A";"b"|])
                                Column.Create("Prop2", [|45; 56|])]) // should be ignored since not presented in the target type
...
let ``Table.ToRows fails when table has no column for a property``() =
    let t = Table.OfColumns ([Column.Create("Prop2", [|45; 56|])]) 
...
let ``Table.ToRows fails when table has different type of column than the property``() =
    let t =
        Table.OfColumns ([Column.Create("TypeString", [|123|]); Column.Create("TypeInt", [|45|]); Column.Create("TypeFloat", [|185.0|]);
                Column.Create("TypeBool", [|true|]); Column.Create("TypeDate", [|System.DateTime(1950, 02, 03)|])])
    t.ToRows<ValidTypesRec>() |> Seq.toArray |> ignore

[<Test; Category("CI"); ExpectedException(typeof<System.ArgumentException>)>]
let ``Table.ToRows fails when target property has invalid type``() =
    let t = Table.OfColumns ([Column.Create("TypeDecimal", [|45; 56|])]) 
...
let TableF_MapiToColumn_ManyArgs() =
    let table:Table = 
        ...
        |> Table.Add(Column.Create("b", [|2|]))  
        |> Table.Add(Column.Create("c", [|3|]))  
        |> Table.Add(Column.Create("d", [|4|]))  
        |> Table.Add(Column.Create("e", [|5|]))  
        |> Table.Add(Column.Create("f", [|6|]))  
...
let TableF_TestAddOneColumn() =
    let data:int[] = [| 1; 2; 4 |]
    let column0:Column = Column.Create("col1", data)
    let table0:Table = Table.Empty
...
let TableF_Transform_1() =
    let table = Table.OfColumns ([Column.Create("x", [|0..10|])])
...
let TableF_Transform_2() =
    let table = Table.OfColumns ([  Column.Create("x", [|0..10|])
                                    Column.Create("y", [| for i in 0..10 do yield float(i) |]) ])
...
let TableF_AppendTransform_1() =
    let table = Table.OfColumns ([Column.Create("x", [|0..10|])])
    let res = table |> Table.AppendTransform ["x"] (fun (x:ImmutableArray<int>) -> Table.OfColumns ([Column.Create ("y", Seq.map float x |> Seq.toArray)]))
...
let TableF_AppendTransform_2() =
    let table = Table.OfColumns ([ Column.Create("x", [|0..10|])
                                   Column.Create("y", [| for i in 0..10 do yield float(i) |]) ])
    let res = table |> Table.AppendTransform ["x";"y"] (fun (x:ImmutableArray<int>) (y:ImmutableArray<float>) -> Table.OfColumns ([Column.Create ("z", Seq.zip x y |> Seq.map (fun (s, t) -> float(s) + t) |> Seq.toArray)]))
...
let ``Table.Add keeps order of columns as FIFO``() =
    let table = Table.Empty |> Table.Add (Column.Create("x", [|1|])) |> Table.Add (Column.Create("y", [|2|]))
    let x = table.[0]
    let y = table.[1]
...
let ``Filters table rows by a single integer column`` (table: Table) (filterBy: int[]) =    
    ...
    let table2 = table |> Table.Add (Column.Create("_filterBy_", filterBy))
    ...
    |> Seq.forall(fun (co, cf) ->
        (match co.Rows with
        | RealColumn v -> Column.Create(co.Name, mask |> Array.map (fun i -> v.Value.[i]))
        | IntColumn v -> Column.Create(co.Name, mask |> Array.map (fun i -> v.Value.[i]))
        | StringColumn v -> Column.Create(co.Name, mask |> Array.map (fun i -> v.Value.[i]))
        | BooleanColumn v -> Column.Create(co.Name, mask |> Array.map (fun i -> v.Value.[i]))
        | DateColumn v -> Column.Create(co.Name, mask |> Array.map (fun i -> v.Value.[i]))) |> areEqualColumnsForSerialization cf)
...
let ``Typed table from untyped``() =
    let t = Table.OfColumns([Column.Create("Lat", [0.0;1.0]); Column.Create("Lon", [0.0;2.0])])
